...
#define FOR_EACH_TIMER(X) \
    ...
    X(ROGUE_RUN_BEGIN) \
    X(ROGUE_RUN_CHOOSE_TRS) \
    X(ROGUE_RUN_CHOOSE_TRAINERS) \
    X(ROGUE_RUN_CHOOSE_RIVAL) \
    X(ROGUE_ADVPATH_GENERATE) \
    X(ROGUE_ADVPATH_GEN_LAYOUT) \
    X(ROGUE_ADVPATH_GEN_ROOM_PLACEMENT) \
    X(TEMP_DEBUG_TIMER) \
