...
#define B_X_ITEMS_BUFF              GEN_3      // In Gen7+, the X Items raise a stat by 2 stages instead of 1.
...
#define B_TRAINERS_KNOCK_OFF_ITEMS  FALSE      // If TRUE, trainers can steal/swap your items (non-berries are restored after battle). In vanilla games trainers cannot steal items.
...
#define B_FLAG_DYNAMAX_BATTLE       FLAG_ROGUE_DYNAMAX_BATTLE   // If this flag is set, the ability to Dynamax in battle is enabled for all trainers.
#define B_FLAG_TERA_ORB_CHARGED     FLAG_ROGUE_TERA_ORB_CHARGED // If this flag is set, the Tera Orb is charged. It is automatically set upon healing and cleared upon Terastallizing once configured.
...
#define B_AFFECTION_MECHANICS           Rogue_EnableAffectionMechanics()      // In Gen6+, there's a stat called affection that can trigger different effects in battle. From LGPE onwards, those effects use friendship instead.
#define B_TRAINER_CLASS_POKE_BALLS      GEN_3 // In Gen7+, trainers will use certain types of Poké Balls depending on their trainer class.
...
#define B_USE_FROSTBITE                 Rogue_GetRevisionModeActive()      // In PLA, Frostbite replaces Freeze. Enabling this flag does the same here. Moves can still be cherry-picked to either Freeze or Frostbite. Freeze-Dry, Secret Power & Tri Attack depend on this config.
...
#define B_TRY_CATCH_TRAINER_BALL        GEN_3 // In Gen4+, trying to catch a Trainer's Pokémon does not consume the Poké Ball.
