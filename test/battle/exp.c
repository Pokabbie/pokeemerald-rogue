...
WILD_BATTLE_TEST("Exp Share(held) gives Experience to mons which did not participate in battle")
{
    ...
    GIVEN {
        ...
        if (item == ITEM_EXP_SHARE)
            EXPECT_GT(GetMonData(&gPlayerParty[1], MON_DATA_EXP), Rogue_ModifyExperienceTables(gSpeciesInfo[SPECIES_WYNAUT].growthRate, 40));
        else
            EXPECT_EQ(GetMonData(&gPlayerParty[1], MON_DATA_EXP), Rogue_ModifyExperienceTables(gSpeciesInfo[SPECIES_WYNAUT].growthRate, 40));
    }
}
