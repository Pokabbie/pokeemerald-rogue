...
const struct TrainerMoney gTrainerMoneyTable[] =
{
    ...
    {TRAINER_CLASS_COMMUNITY_CHALLENGER, 15},
    {TRAINER_CLASS_PAST_ATTEMPT, 15},
                                                                        ...
                                                                        | BATTLE_TYPE_TRAINER_HILL)))
    {
        ...
        for (i = 0; i < monsCount; i++)
        {
            ...
            case F_TRAINER_PARTY_CUSTOM_MOVESET | F_TRAINER_PARTY_HELD_ITEM | F_TRAINER_PARTY_ABILITY:
            {
                ...
                if(trainerNum == TRAINER_STEVEN_CHAMPION && partyData[i].species == SPECIES_METAGROSS)
                {
                    CreateMon(&party[i], partyData[i].species, partyData[i].lvl, fixedIV, TRUE, personalityValue, OT_ID_FORCE_SHINY, 0);
                }
                else
                {
                    CreateMon(&party[i], partyData[i].species, partyData[i].lvl, fixedIV, TRUE, personalityValue, OT_ID_RANDOM_NO_SHINY, 0);
                ...
                for(j = 0; j < ARRAY_COUNT(gBaseStats[partyData[i].species].abilities); j++)
                {
                    u16 ability = gBaseStats[partyData[i].species].abilities[j];

                    if(ability != ABILITY_NONE && partyData[i].ability == ability)
                    {
