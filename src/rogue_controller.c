...
static void ApplyMonPreset(struct Pokemon* mon, const struct RogueMonPreset* preset);
...
void Rogue_ModifyEventMon(struct Pokemon* mon)
{
    ...
    if(presetCount != 0)
    {
        ...
        ApplyMonPreset(mon, &gPresetMonTable[species].presets[presetIndex]);
    }
